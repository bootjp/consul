...
func TestIntentionPrecedenceSorter(t *testing.T) {
	...
	cases := []struct {
		Name     string
		Input    []fields // SSrcNS, SrcN, DstNS, DstN
		Expected []fields // Same structure as Input
	}{
		{
			"exhaustive list",
			[]fields{
...
func TestIntention_String(t *testing.T) {
	...
	cases := map[string]testcase{
		...
		"L4 allow with source peer": {
			...
			partitionPrefix + `peer(billing)/default/foo => ` + partitionPrefix + `default/bar (Precedence: 9, Action: ALLOW)`,
		},
	}
