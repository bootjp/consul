...
| [`consulLogin`](#consullogin) | `object` | optional | Configuration for login to the AWS IAM auth method.  |
...
Configuration for login to the AWS IAM auth method.

| Field | Type | Required | Description |
| ----- | ---- | -------- | ----------- |
| `enabled` | `boolean` | optional | Enable login to Consul's AWS IAM auth method to obtain an ACL token. This requires the auth method to be configured on the Consul server, and the ECS task role must be trusted by the auth method. After login, the token is written to the file `<bootstrapDir>/service-token`.  |
...
| `includeEntity` | `boolean` | optional | Defaults to true. Set this to false to remove the `-aws-include-entity` flag from the `consul login` command. The `-aws-include-entity` flag should be passed if and only if the Consul AWS IAM auth method is configured with `EnableIAMEntityDetails=true`.  |
...
| Field | Type | Required | Description |
| ----- | ---- | -------- | ----------- |
| `kind` | `string` | required | The gateway kind. Must be one of `mesh-gateway`. |
...
| `namespace` | `string` | optional | <EnterpriseAlert inline /> Consul namespace in which the gateway will register .  |
| `partition` | `string` | optional | <EnterpriseAlert inline /> Consul admin partition in which the gateway will register .  |
...
| `address` | `string` | optional |   |
| `port` | `integer` | optional |   |
| `source` | `string` | optional |  Must be one of `publicIP`, `loadBalancerDNS`, `""`, or `null`. |
